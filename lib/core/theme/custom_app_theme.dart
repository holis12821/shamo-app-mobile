import 'package:flutter/material.dart';

class CustomAppTheme {
  /* Color */
  static const kPrimaryColor = Color(0xFF6C5ECF);
  static const kSecondaryMainColor = Color(0xFF38ABBE);
  static const kAlertColor = Color(0xFFED6363);
  static const kDodgerBlue = Color(0xFF2C96F1);
  static const kTaupeGray = Color(0xFF808191);

  static const kRaisinPrimaryColor = Color(0xFF1F1D2B);
  static const kGhostWhite = Color(0xFFF8F8FF);
  static const kAntiFlashWhite = Color(0xFFECEDEF);
  static const kSecondaryColor = Color(0xFF999999);
  static const kStrokeColor = Color(0xFFE1E1E1);
  static const kDavysGray = Color(0xFF504F5E);
  static const kRaisinBlack = Color(0xFF2B2937);
  static const kRaisinBlackLight = Color(0xFF252836);
  static const kMoonstone = Color(0xFF38ABBE);
  static const kTransparentColor = Colors.transparent;
  static const kJet = Color(0xFF3D3B4B);
  static const kRaisinBlackSecond = Color(0xFF242231);
  static const kEmerald = Color(0xFF51C17E);
  static const kSpaceCadet = Color(0xFF2B2844);
  static const kSilver = Color(0xFFC4C4C4);
}
